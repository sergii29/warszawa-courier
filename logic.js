// --- logic.js (v5.4 SMART WEATHER & SAFETY) ---
...
let G = { 
    ...
    lastActive: Date.now(),
    gameTime: 720 // 12:00 (Полдень старт)
};
...
// УЛУЧШЕННАЯ ПОКУПКА (РАЗРЕШАЕТ ПЕРЕПОКУПКУ ПУСТОГО СПРЕЯ)
function buyInvest(type, p) { 
    // Разрешаем покупать, если нет предмета ИЛИ если это спрей и он пустой
    let canBuy = !G[type] || (type === 'spray' && G[type].dur <= 0);
...
function updateUI() {
    try {
        ...
        // --- ДЕНЬ / НОЧЬ (ИСПРАВЛЕНО 21:00 - 06:00) ---
        ...
        let weatherText = "";
        if (weather === "Дождь") {
            weatherText = isNight ? "⛈️ Гроза" : "🌧️ Дождь";
        } else {
            weatherText = isNight ? "🌙 Ясно" : "☀️ Ясно";
        }
        ...
        if(shopList) {
            ...
            for (const [catKey, catName] of Object.entries(categories)) {
                // ФИКС МАГАЗИНА: Если спрей есть, но пуст (dur <= 0), он ДОЛЖЕН появиться
                const items = UPGRADES.filter(u => {
                    if (u.hidden) return false;
                    if (u.cat !== catKey) return false;
                    
                    if (u.id === 'spray') {
                        return !G[u.id] || G[u.id].dur <= 0;
                    }
...
function doWork() {
    try {
        ...
        let nightMult = isNight ? 1.2 : 1.0;
        let gain = 0.10 * Math.max(0.1, G.lvl) * DISTRICTS[G.district].mult * (1 + rankBonus) * bagBonus * nightMult;
        ...
        log("Ошибка в клике: " + e.message, "red");
        console.error(e);
    }
}
...
setInterval(() => {
    try {
        G.gameTime++;
        if (G.gameTime >= 1440) G.gameTime = 0; 
