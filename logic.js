// --- logic.js ---
// VERSION: 35.0 (GOD MODE + STABILITY)
// Ключ сохранения: WARSZAWA_FOREVER
// 1. Описания и иконки на месте (не пропадут).
// 2. Цены и коэффициенты берутся из Админки.
// 3. Сфера работает, заказы генерируются.
...
// === СТРУКТУРА (Имена и описания - СТАБИЛЬНЫЕ) ===
const STATIC_BUSINESS_META = [
    { id: 'vending', name: 'Vending Machine', icon: '🍫', desc: 'Автомат со снеками. Пассивный доход.', type: 'maintenance' },
    { id: 'vege', name: 'Warzywniak', icon: '🥦', desc: 'Овощная лавка. Товар может испортиться.', type: 'lottery' },
    { id: 'kebab', name: 'Kebab u Aliego', icon: '🥙', desc: 'Точка с кебабом. Нужен продавец.', type: 'strategy' },
    { id: 'zabka', name: 'Żabka Franchise', icon: '🐸', desc: 'Франшиза. Высокие обороты.', type: 'high_stakes' }
];

const STATIC_UPGRADES_META = [
    { id: 'starter_bag', name: 'Старый Рюкзак', icon: '🎒', desc: 'Лучше, чем в руках.', bonus: '+2% PLN', hidden: true },
    { id: 'starter_phone', name: 'Древний Телефон', icon: '📱', desc: 'Звонит и ладно.', bonus: 'Связь', hidden: true },
    { id: 'bag', name: 'Термосумка', icon: '🎒', desc: '+15% к выплатам.', bonus: '+15% PLN' }, 
    { id: 'phone', name: 'Смартфон Pro', icon: '📱', desc: 'Заказы чаще.', bonus: 'Заказы x1.4' }, 
    { id: 'scooter', name: 'Электросамокат', icon: '🛴', desc: 'Расход энергии -30%.', bonus: '⚡ -30%' },
    { id: 'helmet', name: 'Шлем Safety', icon: '🧢', desc: 'Риск аварии -50%.', bonus: '🛡️ Безопасность' },
    { id: 'raincoat', name: 'Дождевик', icon: '🧥', desc: 'Защита от дождя.', bonus: '☔ Сухость' },
    { id: 'powerbank', name: 'Powerbank 20k', icon: '🔋', desc: 'Автопилот дольше.', bonus: '🤖 +50% времени' }
];

// === НАСТРОЙКИ (Цифры - ИЗМЕНЯЕМЫЕ ЧЕРЕЗ АДМИНКУ) ===
const DEFAULT_SETTINGS = {
    ...
    // МАССИВЫ (ЭТИ ЦИФРЫ МЕНЯЮТСЯ В АДМИНКЕ)
    districts_data: [
        { housePrice: 250000, czynszBase: 25, mult: 1, rentPct: 0.05, minLvl: 0 },       
        { housePrice: 850000, czynszBase: 80, mult: 1.5, rentPct: 0.10, minLvl: 2.5 }, 
        { housePrice: 3500000, czynszBase: 250, mult: 1.55, rentPct: 0.15, minLvl: 5.0 } 
    ],
    business_data: [
        { basePrice: 5000, minLvl: 5.0, stockPrice: 2, sellPrice: 5, empPrice: 150 },
        { basePrice: 20000, minLvl: 10.0, stockPrice: 5, sellPrice: 12, empPrice: 300 },
        { basePrice: 75000, minLvl: 20.0, stockPrice: 10, sellPrice: 25, empPrice: 500 },
        { basePrice: 300000, minLvl: 30.0, stockPrice: 50, sellPrice: 120, empPrice: 1500 }
    ]
};
...
// Слияние статичных данных и настроек админа
function getBusinessConfig(id) {
    const metaIndex = STATIC_BUSINESS_META.findIndex(b => b.id === id);
    if(metaIndex === -1) return null;
    const meta = STATIC_BUSINESS_META[metaIndex];
    const data = SETTINGS.business_data[metaIndex] || DEFAULT_SETTINGS.business_data[metaIndex];
    
    let mult = 1 + (Math.max(1.0, G.lvl) * 0.2);
    let currentBasePrice = parseFloat((data.basePrice * mult).toFixed(2));
    
    return { ...meta, ...data, currentBasePrice };
}

function getDistrictConfig(id) {
    const staticNames = ["Praga", "Mokotów", "Śródmieście"];
    const name = staticNames[id] || "Unknown";
    const data = SETTINGS.districts_data[id] || DEFAULT_SETTINGS.districts_data[id];
    return { name, ...data };
}
...
function renderBusiness() {
    ...
    let totalCash = 0;
    let totalVal = 0;
    Object.values(G.business).forEach(b => { totalCash += (b.cash || 0); totalVal += 1000; });
    ...
    STATIC_BUSINESS_META.forEach((meta, idx) => {
        const conf = getBusinessConfig(meta.id);
        if (!G.business[meta.id]) G.business[meta.id] = null; 
        let userBiz = G.business[meta.id];
        let isOwned = !!userBiz;
        let hasLvl = G.lvl >= conf.minLvl;

        if (!isOwned) {
            let reason = "";
            if (!hasHouse) reason = "🔒 НУЖНА КВАРТИРА";
            else if (!hasLvl) reason = `🔒 НУЖЕН LVL ${conf.minLvl}`;
            else reason = `КУПИТЬ ${conf.currentBasePrice.toLocaleString()} PLN`;
            ...
            let btnAction = canBuy ? `onclick="buyBusiness('${meta.id}')"` : "";

            html += `<div class="biz-card biz-locked">
                <div class="biz-header"><div style="display:flex; align-items:center;"><div class="biz-icon">${meta.icon}</div><div><div class="biz-title">${meta.name}</div><div style="font-size:10px; color:#aaa;">${meta.desc}</div></div></div></div>
            ...
            html += `<div class="biz-card" onclick="openBusinessModal('${meta.id}')">
                <div class="biz-header">
                    <div style="display:flex; align-items:center;">
                        <div class="biz-icon">${meta.icon}</div>
                        <div>
                            <div class="biz-title">${meta.name} <span class="biz-level">LVL ${userBiz.level||1}</span></div>
...
function buyBusiness(id) {
    ...
    let conf = getBusinessConfig(id);
    if (G.lvl < conf.minLvl) { log(`⛔ Нужен уровень ${conf.minLvl}!`, "var(--danger)"); return; }
    
    if (G.money >= conf.currentBasePrice) {
        G.money = parseFloat((G.money - conf.currentBasePrice).toFixed(2));
        G.business[id] = { stock: 10, cash: 0, empTime: 0, level: 1 };
        addHistory('🏗️ БИЗНЕС', conf.currentBasePrice, 'minus');
        tg.HapticFeedback.notificationOccurred('success');
        save(); updateUI();
    } else { log(`Не хватает денег! Нужно ${conf.currentBasePrice} PLN`, "var(--danger)"); tg.HapticFeedback.notificationOccurred('error'); }
}

function openBusinessModal(id) {
    currentBizId = id;
    const conf = getBusinessConfig(id);
    
    document.getElementById('bm-title').innerText = conf.name;
    document.getElementById('bm-desc').innerText = conf.desc;
...
function updateBusinessModalUI() {
    ...
    const conf = getBusinessConfig(currentBizId);
    ...
    if (uBiz.empTime > 0) {
        ...
        document.getElementById('hire-price').innerText = conf.empPrice + " PLN";
    }
...
function manualSell() {
    ...
    if (uBiz.stock > 0) {
        if (G.en >= 5) {
            G.en -= 5;
            uBiz.stock--;
            let profit = conf.sellPrice;
            let tax = profit * SETTINGS.economy.business_tax;
            uBiz.cash += (profit - tax);
            updateBusinessModalUI();
            updateUI();
            tg.HapticFeedback.impactOccurred('light');
        } else { log("Нужна энергия!", "var(--danger)"); }
    } else { log("Склад пуст! Закупите товар.", "var(--danger)"); triggerShake(); }
}
...
function withdrawFromModal() {
    ...
    if (uBiz.cash > 0) {
        ...
        updateBusinessModalUI();
        updateUI(); save(); closeBusinessModal();
    } else { log("Касса пуста.", "var(--danger)"); }
}
...
function doWork() {
    ...
    // Запускаем генерацию заказа при клике, если его нет
    ...
    STATIC_UPGRADES_META.forEach(up => { 
    ...
    let distConf = getDistrictConfig(G.district);
    let distMult = distConf.mult || 1;
...
function generateOrder() { 
    ...
    let distConf = getDistrictConfig(G.district);
    let baseRew = (base + d * perKm) * Math.max(0.1, G.lvl) * distConf.mult * (weather === "Дождь" ? 1.5 : 1);
...
function buyInvest(type, priceKey) { 
    ...
    if(!G[type] && G.money >= price) { 
        ...
        let maxDur = 100;
        let conf = STATIC_UPGRADES_META.find(u => u.id === type);
...
function sellInvest(type) {
    if(G[type]) {
        let conf = STATIC_UPGRADES_META.find(u => u.id === type);
        let priceKey = conf ? (conf.priceKey || type) : null; // Фикс для предметов без priceKey
        if (!priceKey) return; 
...
function checkMilestones() {
    ...
    G.activeMilestones.forEach(m => {
        let cur = 0;
        if(m.type === 'orders') cur = G.totalOrders;
        if(m.type === 'bottles') cur = G.totalBottles;
        if(m.type === 'clicks') cur = G.totalClicks;
    });
}
...
function validateInventory() { STATIC_UPGRADES_META.forEach(up => { if(G[up.id] && G[up.id].dur > 100) G[up.id].dur = 100; }); }
    ...
    if(window.db) {
        window.db.ref('game_settings').on('value', (snapshot) => {
            ...
            if (serverSettings) {
                ...
                if (serverSettings.districts_data) SETTINGS.districts_data = serverSettings.districts_data;
                if (serverSettings.business_data) SETTINGS.business_data = serverSettings.business_data;
...
function updateUI() {
    try {
        ...
        let distConf = getDistrictConfig(G.district);
        document.getElementById('district-ui').innerText = "📍 " + distConf.name;
        ...
        if (!isBroken) {
            ...
            let distConf = getDistrictConfig(G.district);
            let distMult = distConf.mult || 1;
            let rate = (baseC * Math.max(0.1, G.lvl) * distMult * (1 + rankBonus)).toFixed(2);
        ...
        if(itemsList) {
            STATIC_UPGRADES_META.forEach(up => {
                if(G[up.id]) {
                    ...
                    let repairCost = getRepairCost(up.repairKey || up.id);
        ...
        if(distCont) {
            SETTINGS.districts_data.forEach((d, i) => {
                let conf = getDistrictConfig(i);
                let isCurrent = G.district === i;
                let isOwner = G.housing && G.housing.id === i;
                let actionBtn = isCurrent ? `<button class="btn-action btn-secondary" style="opacity:0.7;">ТУТ</button>` : `<button class="btn-action" onclick="moveDistrict(${i})">ЕХАТЬ ${conf.price||0}</button>`;
                if(isOwner) actionBtn = `<button class="btn-action" style="background:var(--gold); color:black;">ВЛАДЕЛЕЦ</button>`;
                else if(conf.housePrice) actionBtn += `<button class="btn-action" style="margin-top:5px; border:1px solid var(--accent-gold); background:transparent; color:var(--accent-gold);" onclick="buyHouse(${i})">КУПИТЬ ${(conf.housePrice/1000).toFixed(0)}k</button>`;
                distHTML += `<div class="card"><b>${conf.name}</b><small>Аренда: ${(conf.rentPct*100).toFixed(0)}% | x${conf.mult}</small>${actionBtn}</div>`;
            });
            distCont.innerHTML = distHTML;
        }

        renderBankFull(); renderMilestones();
        ...
        let distConf = getDistrictConfig(G.district);
        let rentVal = (G.housing && G.housing.id === G.district) ? getDynamicPrice(distConf.czynszBase) : (G.money * distConf.rentPct);
...
setInterval(() => {
    if (G.money > 0) {
        ...
        G.rent--; if(G.rent <= 0) {
            let distConf = getDistrictConfig(G.district);
            let cost = (G.housing && G.housing.id === G.district) ? getDynamicPrice(distConf.czynszBase) : (G.money * distConf.rentPct);
        ...
        Object.keys(G.business).forEach(bid => {
            if(G.business[bid] && G.business[bid].empTime > 0) {
                ...
                if(G.business[bid].empTime % 5 === 0 && G.business[bid].stock > 0) {
                    ...
                    let conf = getBusinessConfig(bid);
                    let profit = conf.sellPrice * (1 - SETTINGS.economy.business_tax);
