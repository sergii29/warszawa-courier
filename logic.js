// --- logic.js ---
// VERSION: 8.1 (FIXED SPHERE & INPUTS)
// Исправлено залипание кнопки. Логика Админки сохранена полностью.
...
// === FIX: ВОССТАНОВЛЕНА РАБОТА СФЕРЫ ===
...
if(sphere) {
    ...
    sphere.addEventListener('touchstart', (e) => { 
        e.preventDefault(); // Предотвращает дублирование мышью
        try { tg.HapticFeedback.impactOccurred('medium'); } catch(err){}
        doWork(); 
    }, {passive: false});

    // Надежный обработчик для ПК (работает, если не сработал тач)
